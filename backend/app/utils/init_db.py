...
def create_tables():
    """Создает таблицы с retry механизмом."""
    max_retries = 5
    for attempt in range(max_retries):
        try:
            ...
            print(f"❌ Ошибка при создании таблиц (попытка {attempt + 1}/{max_retries}): {e}")
            if attempt < max_retries - 1:
                time.sleep(2 ** attempt)  # Exponential backoff
            else:
                raise
        except Exception as e:
            print(f"❌ Неожиданная ошибка при создании таблиц: {e}")
            raise

def create_admin():
    """Создает администратора с retry механизмом."""
    max_retries = 3
    for attempt in range(max_retries):
        ...
        try:
            ...
            print(f"❌ Ошибка при создании админа (попытка {attempt + 1}/{max_retries}): {e}")
            db.rollback()
            if attempt < max_retries - 1:
                time.sleep(2)
            else:
            ...
            print(f"❌ Неожиданная ошибка при создании админа: {e}")
            db.rollback()
            raise
        finally:
            db.close()
...
def create_demo_data():
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            print(f"❌ Ошибка при создании демо-данных (попытка {attempt + 1}/{max_retries}): {str(e)}")
            db.rollback()
            if attempt < max_retries - 1:
                time.sleep(3)
            else:
                print(f"❌ Не удалось создать демо-данные после {max_retries} попыток")
                raise
        except Exception as e:
            print(f"❌ Неожиданная ошибка при создании демо-данных: {str(e)}")
            db.rollback()
            raise
        finally:
            db.close()
